...
创建、组织和管理 Helm chart 以打包和部署 Kubernetes 应用程序的综合指南。

## 目的

此技能提供构建生产就绪 Helm chart 的分步说明,包括 chart 结构、模板模式、值管理和验证策略。
...
apiVersion: v2
name: my-app
description: My Application 的 Helm chart
type: application
...
1. **使用语义化版本控制** 用于 chart 和应用版本
2. **记录所有值** 在 values.yaml 中使用注释
3. **使用模板辅助函数** 用于重复逻辑
4. **在打包前验证 chart**
5. **显式固定依赖版本**
6. **使用条件** 用于可选资源
7. **遵循命名约定**(小写、连字符)
8. **包含 NOTES.txt** 附带使用说明
9. **使用辅助函数一致地添加标签**
10. **在所有环境中测试安装**
...
- `references/chart-structure.md` - 详细的 chart 组织
