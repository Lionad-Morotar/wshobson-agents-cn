...
description: 为语义搜索和 RAG 应用选择和优化嵌入模型。在选择嵌入模型、实现分块策略或针对特定领域优化嵌入质量时使用。
---
...
| 模型                       | 维度    | 最大 Token 数 | 最适合场景                          |
| -------------------------- | ------- | ------------- | ----------------------------------- |
| **voyage-3-large**         | 1024    | 32000         | Claude 应用(Anthropic 推荐)         |
| **voyage-3**               | 1024    | 32000         | Claude 应用,成本优化                |
...
| **text-embedding-3-small** | 1536    | 8191          | OpenAI 应用,成本优化                |
...
# 初始化 Voyage AI 嵌入(Anthropic 推荐用于 Claude)
...
# 使用 Matryoshka 嵌入进行降维
def get_reduced_embedding(text: str, dimensions: int = 512) -> List[float]:
    """获取降低维度的嵌入向量(Matryoshka 技术)。"""
...
- **批量请求**: 比逐个处理更高效
- **缓存嵌入**: 避免为静态内容重新计算
- **在 Claude 应用中使用 Voyage AI**: Anthropic 推荐
...
- [LangChain 嵌入模型](https://python.langchain.com/docs/integrations/text_embedding/)
