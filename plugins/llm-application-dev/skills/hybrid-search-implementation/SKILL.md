...
def reciprocal_rank_fusion(
    ...
    Args:
        result_lists: 每个搜索方法的 (doc_id, score) 元组列表
        k: RRF 常数(值越大,低排名权重越高)
        weights: 每个结果列表的可选权重

    Returns:
        融合后的排名,返回 (doc_id, score) 元组
    """
    if weights is None:
...
def linear_combination(
    ...
    Args:
        vector_results: 向量搜索的 (doc_id, similarity_score)
        keyword_results: 关键词搜索的 (doc_id, bm25_score)
        alpha: 向量搜索的权重(1-alpha 为关键词权重)
    """
    # 将分数归一化到 [0, 1]
