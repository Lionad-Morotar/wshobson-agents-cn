...
description: 掌握使用 Playwright 和 Cypress 进行端到端测试，以构建可靠的测试套件，捕获错误、提高信心并实现快速部署。在实施 E2E 测试、调试不稳定的测试或建立测试标准时使用。
---

# E2E 测试模式

构建可靠、快速且可维护的端到端测试套件，提供快速发布代码并在用户之前发现回归问题的信心。
...
1. **使用数据属性**：`data-testid` 或 `data-cy` 用于稳定选择器
2. **避免脆弱选择器**：不要依赖 CSS 类或 DOM 结构
3. **测试用户行为**：点击、输入、查看 - 而非实施细节
4. **保持测试独立**：每个测试应隔离运行
...
- **references/flaky-test-debugging.md**：调试不可靠的测试
- **assets/e2e-testing-checklist.md**：使用 E2E 测试什么
- **assets/selector-strategies.md**：查找可靠的选择器
- **scripts/test-analyzer.ts**：分析测试不稳定性持续时间
