...
description: 掌握使用 Turborepo、Nx 和 pnpm workspaces 进行单体仓库管理，以构建高效、可扩展的多包仓库，实现优化的构建和依赖管理。在设置单体仓库、优化构建或管理共享依赖时使用。
---

# 单体仓库管理

构建高效、可扩展的单体仓库，实现代码共享、一致的工具和跨多个包和应用程序的原子更改。
...
1. **一致的版本控制**：在工作空间中锁定依赖版本
2. **共享配置**：集中化 ESLint、TypeScript、Prettier 配置
3. **依赖图**：保持无环，避免循环依赖
4. **有效缓存**：正确配置输入/输出
5. **类型安全**：在前端/后端之间共享类型
6. **测试策略**：包中的单元测试，应用中的 E2E
7. **文档**：每个包中的 README
...
- **循环依赖**：A 依赖 B，B 依赖 A
- **幻影依赖**：使用不在 package.json 中的依赖
- **不正确的缓存输入**：Turborepo 输入中缺少文件
- **过度共享**：共享应该分开的代码
- **共享不足**：跨包复制代码
- **大型单体仓库**：没有适当的工具，构建会变慢
...
- **references/turborepo-guide.md**：全面的 Turborepo 文档
- **references/nx-guide.md**：Nx 单体仓库模式
- **references/pnpm-workspaces.md**：pnpm workspace 功能
- **assets/monorepo-checklist.md**：设置清单
- **assets/migration-guide.md**：多仓库到单体仓库迁移
