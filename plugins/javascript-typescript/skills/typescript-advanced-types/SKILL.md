...
掌握 TypeScript 高级类型系统的综合指南,包括泛型、条件类型、映射类型、模板字面量类型和实用类型,用于构建健壮的类型安全应用程序。
...
const auto = identity(true); // 推断类型: boolean
```
...
1. **使用 `unknown` 而非 `any`**: 强制类型检查
2. **优先使用 `interface` 定义对象形状**: 更好的错误消息
3. **使用 `type` 定义联合类型和复杂类型**: 更灵活
4. **利用类型推断**: 尽可能让 TypeScript 推断
5. **创建辅助类型**: 构建可复用的类型工具
6. **使用 const 断言**: 保留字面量类型
7. **避免类型断言**: 使用类型守卫替代
8. **记录复杂类型**: 添加 JSDoc 注释
...
- **TypeScript 深度解析**: https://basarat.gitbook.io/typescript/
- **Effective TypeScript**: Dan Vanderam 著
