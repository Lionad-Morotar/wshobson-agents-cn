...
class CommandBus:
    ...
    async def dispatch(self, command: Command) -> Any:
        ...
        if not handler:
            raise ValueError(f"{type(command).__name__} 无处理器")
...
class QueryBus:
    ...
    async def dispatch(self, query: Query) -> Any:
        ...
        if not handler:
            raise ValueError(f"{type(query).__name__} 无处理器")
...
- [Microsoft CQRS 指南](https://docs.microsoft.com/en-us/azure/architecture/patterns/cqrs)