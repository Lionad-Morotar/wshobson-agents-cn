...
description: Python 错误处理模式,包括输入验证、异常层次结构和部分失败处理。在实现验证逻辑、设计异常策略、处理批处理失败或构建健壮的 API 时使用。
---

# Python 错误处理

构建健壮的 Python 应用程序,包含适当的输入验证、有意义的异常和优雅的失败处理。良好的错误处理使调试更容易,系统更可靠。
...
在执行昂贵操作之前尽早验证输入。尽可能一次性报告所有验证错误。

### 2. 有意义的异常

使用适当的异常类型并附带上下文。消息应说明失败的原因、原因以及如何修复。

### 3. 部分失败

在批量操作中,不要让单个失败中止所有操作。分别跟踪成功和失败。
...
使用 Pydantic 模型进行结构化输入验证,自动生成错误消息。
...
适当地使用 Python 内置的异常类型,根据需要添加上下文。

| 失败类型 | 异常 | 示例 |
|--------------|-----------|---------|
| 无效输入 | `ValueError` | 错误的参数值 |
| 错误类型 | `TypeError` | 期望字符串,得到整数 |
| 缺失项 | `KeyError` | 未找到字典键 |
...
1. **尽早验证** - 在昂贵操作之前检查输入
2. **使用特定异常** - `ValueError`、`TypeError`,而非通用的 `Exception`
3. **包含上下文** - 消息应说明是什么、为什么以及如何修复
4. **在边界转换类型** - 尽早将字符串解析为枚举/域类型
5. **链接异常** - 使用 `raise ... from e` 保留调试信息
6. **处理部分失败** - 不要因为单项错误而中止批处理
7. **使用 Pydantic** - 用于具有结构化错误的复杂输入验证
