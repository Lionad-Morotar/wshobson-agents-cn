...
构建容错的 Python 应用程序，优雅地处理临时故障、网络问题和服务中断。弹性模式在依赖项不可靠时保持系统运行。
...
重试临时错误（网络超时、临时服务问题）。不要重试永久错误（无效凭据、错误请求）。
...
使用 `tenacity` 库实现生产级重试逻辑。对于简单场景，可考虑内置重试功能或轻量级自定义实现。
...
将特定的临时异常列入白名单。永远不要重试：

- `ValueError`、`TypeError` - 这些是错误，不是临时问题
- `AuthenticationError` - 无效凭据不会变为有效
- HTTP 4xx 错误（429 除外）- 客户端错误是永久的
