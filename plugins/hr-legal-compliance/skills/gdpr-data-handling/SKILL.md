...
description: 实施符合 GDPR 的数据处理，包括同意管理、数据主体权利和隐私设计。在构建处理欧盟个人数据的系统、实施隐私控制或进行 GDPR 合规审查时使用。
---

# GDPR 数据处理

符合 GDPR 的数据处理、同意管理和隐私控制的实用实施指南。
...
class DSARHandler:
    ...
    EXTENSION_ALLOWED_DAYS = 60  # 用于复杂请求
...
class PrivacyFirstDataModel:
    ...
    # 将 PII 与行为数据分离
    user_profile_schema = {
        'user_id': str,  # UUID，不是顺序的
        'email_hash': str,  # 哈希用于查找
        'created_at': datetime,
        # 最小化数据收集
        'preferences': {
            'language': str,
    ...
    # 假名化行为数据
    analytics_schema = {
        'session_id': str,  # 不链接到 user_id
        'pseudonym_id': str,  # 轮换假名
        'events': list,
        'device_category': str,  # 通用化，不具体
        'country': str,  # 不是城市级别
    }
...
- [EDPB 指南](https://edpb.europa.eu/our-work-tools/general-guidance/gdpr-guidelines-recommendations-best-practices_en)
