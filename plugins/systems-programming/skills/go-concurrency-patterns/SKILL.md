...
Go 并发编程的生产级模式，包括 goroutine、channel、同步原语和 context 管理。
...
- **使用 context** - 用于取消和超时控制
- **关闭 channel** - 仅在发送端关闭
- **使用 errgroup** - 用于带错误的并发操作
- **缓冲 channel** - 当知道数量时使用
- **优先使用 channel** - 尽可能使用 channel 而非互斥锁

### 不应该做的

- **不要泄漏 goroutine** - 始终提供退出路径
- **不要在接收端关闭** - 会导致 panic
- **不要使用共享内存** - 除非必要
- **不要忽略 context 取消** - 检查 ctx.Done()
- **不要使用 time.Sleep 进行同步** - 使用正确的同步原语
