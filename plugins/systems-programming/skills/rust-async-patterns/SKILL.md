...
description: 精通 Rust 异步编程，包括 Tokio、异步 trait、错误处理和并发模式。在构建异步 Rust 应用、实现并发系统或调试异步代码时使用。
---

# Rust 异步编程模式

使用 Tokio 运行时的生产级 Rust 异步编程模式，包括任务、通道、流和错误处理。
...
- **优先使用通道** - 尽可能避免共享状态
- **使用 `JoinSet`** - 用于管理多个任务
- **使用 tracing 追踪** - 用于调试异步代码
- **处理取消** - 检查 `CancellationToken`

### 不推荐做法

- **不要阻塞** - 永远不要在异步代码中使用 `std::thread::sleep`
- **不要跨 await 持有锁** - 会导致死锁
- **不要无限制派生** - 使用信号量进行限制
- **不要忽略错误** - 使用 `?` 传播或记录
- **不要忘记 Send 边界** - 对于派生的 future

## 资源

- [Tokio 教程](https://tokio.rs/tokio/tutorial)
- [Async 书籍](https://rust-lang.github.io/async-book/)
