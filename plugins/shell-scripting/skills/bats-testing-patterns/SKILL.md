...
description: 掌握 Bash 自动化测试系统 (Bats) 以全面测试 Shell 脚本。在为 Shell 脚本编写测试、CI/CD 流水线或需要脚本测试驱动开发时使用。
---

# Bats 测试模式

使用 Bats (Bash 自动化测试系统) 为 Shell 脚本编写全面单元测试的综合指南，包括测试模式、fixtures 和生产级 Shell 测试的最佳实践。
...
Bats (Bash 自动化测试系统) 是一个符合 TAP (测试任何协议) 的 Shell 脚本测试框架，它提供：

- 简单、自然的测试语法
- 与 CI 系统兼容的 TAP 输出格式
- Fixtures 和 setup/teardown 支持
- 断言辅助函数
- 并行测试执行
...
1. **每个测试只测试一件事** - 单一职责原则
2. **使用描述性测试名称** - 清楚地说明正在测试什么
3. **测试后清理** - 始终在 teardown 中删除临时文件
4. **测试成功和失败路径** - 不要只测试正常流程
5. **模拟外部依赖** - 隔离被测单元
6. **为复杂数据使用 fixtures** - 使测试更具可读性
