---
name: web3-testing
description: 使用 Hardhat 和 Foundry 通过单元测试、集成测试和主网分叉全面测试智能合约。在测试 Solidity 合约、设置区块链测试套件或验证 DeFi 协议时使用。
---

# Web3 智能合约测试

掌握使用 Hardhat、Foundry 和高级测试模式全面测试智能合约的策略。
...
1. **测试覆盖率**：目标是 >90% 覆盖率
2. **边缘情况**：测试边界条件
3. **Gas 限制**：验证函数不会达到区块 Gas 限制
4. **重入攻击**：测试重入漏洞
5. **访问控制**：测试未授权的访问尝试
6. **事件**：验证事件发出
7. **Fixture**：使用 fixture 避免代码重复
8. **主网分叉**：使用真实合约测试
9. **模糊测试**：使用基于属性的测试
10. **CI/CD**：在每次提交时自动测试
