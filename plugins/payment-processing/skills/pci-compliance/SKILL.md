...
掌握 PCI DSS（支付卡行业数据安全标准）合规要求，确保安全处理支付数据和持卡人信息。
...
**Level 1**: > 600 万笔交易/年（需要年度 ROC）
**Level 2**: 100-600 万笔交易/年（需要年度 SAQ）
**Level 3**: 2万-100万笔电子商务交易/年
**Level 4**: < 2万笔电子商务交易或 < 100万笔总交易
...
- **references/data-minimization.md**: 永不存储禁止的数据
...
1. **存储 CVV**: 永不存储卡验证码
2. **未加密的 PAN**: 卡号必须在静态时加密
3. **弱加密**: 使用 AES-256 或同等强度的加密
4. **无访问控制**: 限制可以访问持卡人数据的人员
...
2. **令牌化**: 用令牌替换卡数据
3. **网络分段**: 隔离持卡人数据环境
4. **外包**: 使用符合 PCI 标准的支付处理器
5. **不存储**: 永不存储完整的卡详细信息

通过最小化接触卡数据的系统，您可以显著减少合规负担。
