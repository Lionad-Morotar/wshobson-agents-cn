...
description: 精通 React、Vue 和 Svelte 组件模式，包括 CSS-in-JS、组合策略和可复用组件架构。在构建 UI 组件库、设计组件 API 或实现前端设计系统时使用。
---

# Web 组件设计

使用现代框架构建可复用、可维护的 UI 组件，采用清晰的组合模式和样式方法。
...
| **CSS Modules**     | 作用域 CSS 文件        | 现有 CSS、渐进式采用          |
...
function useAccordion() {
  ...
  if (!context) throw new Error("必须在 Accordion 内使用");
  return context;
}
...
- **无障碍缺失**：使用屏幕阅读器和键盘导航测试
- **打包体积**：Tree-shake 未使用的组件变体

## 资源

- [React 组件模式](https://reactpatterns.com/)
- [Vue 组合式 API 指南](https://vuejs.org/guide/reusability/composables.html)
- [Svelte 组件文档](https://svelte.dev/docs/svelte-components)
- [Radix UI 基础组件](https://www.radix-ui.com/primitives)
